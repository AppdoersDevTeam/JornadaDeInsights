-- Drop entire localhost dev sessions from analytics totals (including first-page hits without localhost referrer).
...
localhost_sessions as (
  select distinct session_id
  from public.site_page_views
  where created_at >= (select since_ts from range_start)
    and (
      coalesce(referrer, '') ~* '(localhost|127\.0\.0\.1)'
      or coalesce(page_url, '') ~* '(localhost|127\.0\.0\.1)'
    )
),
filtered as (
  ...
  from public.site_page_views
  where created_at >= (select since_ts from range_start)
    and session_id not in (select session_id from localhost_sessions)
),
page_counts as (
