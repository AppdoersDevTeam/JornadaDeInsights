...
// Same token verification as requireAdmin, without the admin-email gate.
// Use for endpoints any authenticated user may call (e.g. notifications list/mark-read).
export const requireUser = async (req, res) => {
  ...
  const allowedAdmins = getAllowedAdmins();
  const isAdmin = allowedAdmins.includes((user.email || '').toLowerCase());

  return { user, supabaseAdmin, isAdmin };
};
